...
async function main(): Promise<void> {
  logger.info('=== Kalshi BTC Latency Arbitrage Bot Starting ===');
  logger.info('Configuration loaded', {
    dryRun: config.dryRun,
    marketTicker: config.kalshi.marketTicker,
    pollIntervalMs: config.strategy.pollIntervalMs,
    priceMoveThresholdPct: config.strategy.priceMoveThresholdPct,
    kalshiEdgeThreshold: config.strategy.kalshiEdgeThreshold,
    maxPositionSize: config.strategy.maxPositionSize,
    maxOpenOrders: config.strategy.maxOpenOrders,
    trendWindowSeconds: config.strategy.trendWindowSeconds,
  });

  if (config.dryRun) {
    logger.warn('DRY RUN MODE ENABLED — no real orders will be placed');
  }

  // Instantiate core components
  const coinbaseClient = new CoinbaseClient();
  const kalshiClient = new KalshiClient();
  const strategy = new ArbStrategy(kalshiClient);

  // Current Kalshi ticker in use (supports auto-rotation).
  let currentMarketTicker = config.kalshi.marketTicker;


  // Shared state between the Kalshi fetch loop and the evaluation handler.
  let latestMarket: KalshiMarket | null = null;
  let isEvaluating = false;
  let fetchLoopActive = true;
  let balanceLogTimer: NodeJS.Timeout | null = null;
  ...
  async function evaluateOnTick(coinbaseData: CoinbasePriceData): Promise<void> {
    if (!latestMarket) {
      logger.debug('No Kalshi market data yet — waiting for first fetch');
      return;
    }
    if (isEvaluating) return;
    isEvaluating = true;
    try {
      logger.debug('Market state', {
      ...
      if (result.action !== 'hold') {
        ...
        logger.debug('Strategy holding', { reason: result.reason });
      }
    } catch (err) {
  ...
  coinbaseClient.on('priceUpdate', (data: CoinbasePriceData) => {
  ...
  async function runKalshiFetchLoop(): Promise<void> {
    while (fetchLoopActive) {
      try {
        ...
        try {
          ...
          const isPending =
            strategy.getPendingPhase() !== null && strategy.getPendingSide() !== null;
        ...
        if (is404) {
          ...
          try {
            ...
            const best = openMarkets
              ...
              .sort((a, b) => new Date(a.close_time).getTime() - new Date(b.close_time).getTime())[0];
            if (best) {
              logger.warn('Recovered market ticker after 404', { from: currentMarketTicker, to: best.ticker });
              currentMarketTicker = best.ticker;
              latestMarket = best;
            } else {
              logger.warn('No open market found for series — waiting 5s', { series: prefix });
          ...
          logger.warn('Failed to fetch Kalshi market — retrying', { error: msg });
  ...
  async function logBalance(): Promise<void> {
    try {
      const balanceCents = config.dryRun
        ? strategy.getDemoCashCents()
        : await kalshiClient.getBalance();
      cachedBalanceCents = balanceCents;
      if (cachedStartBalanceCents === 0) cachedStartBalanceCents = balanceCents;
  ...
  async function logStatus(): Promise<void> {
    ...
    if (config.dryRun) {
      ...
      if (cachedStartBalanceCents === 0) cachedStartBalanceCents = strategy.getDemoStartingCashCents();
    ...
    const posSide =
      cachedPositionContracts > 0 ? 'LONG' : cachedPositionContracts < 0 ? 'SHORT' : 'FLAT';
    const diffPct =
      underlying && underlying > 0 && diff !== null
    ...
    logger.info('── STATUS ──', {
      ...
      cross: crossDir
        ...
        : '—',
      armedTP:
        tpCents !== null ? `${tpSide?.toUpperCase()} @ ${tpCents}¢` : '—',
  ...
  balanceLogTimer = setInterval(() => {
    if (coinbaseConnected && kalshiReady) void logBalance();
  }, 60_000);

  statusTimer = setInterval(() => {
    if (coinbaseConnected && kalshiReady) void logStatus();
  }, 2_000);
  ...
  async function shutdown(signal: string): Promise<void> {
    ...
    if (!config.dryRun) {
      ...
      if (openIds.length > 0) {
        logger.info('[DRY RUN] Would cancel open orders on shutdown', { orderIds: openIds });
      }
    }
