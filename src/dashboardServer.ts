...
import { BotRunner, type BotMode } from './botRunner.js';
...
function httpStatusFromKalshiError(err: unknown): number | null {
  const msg = err instanceof Error ? err.message : String(err);
  if (/\[401\]|status code 401|Unauthorized/i.test(msg)) return 401;
  if (/\[403\]|status code 403|Forbidden/i.test(msg)) return 403;
  if (/\[404\]|status code 404|Not Found/i.test(msg)) return 404;
  if (/\[(?:429|408)\]|status code 429|status code 408|Too Many|timeout/i.test(msg)) return 503;
  if (/\[5\d\d\]|status code 5/i.test(msg)) return 502;
  return null;
}

async function fetchCoinbaseBtcUsdSpot(): Promise<number> {
  const resp = await axios.get<{ price?: string }>('https://api.exchange.coinbase.com/products/BTC-USD/ticker', {
    timeout: 8_000,
  });
        ...
        <div style="min-width: 260px;">
          ...
          <div style="min-width: 260px;">
            ...
            <input
              ...
              type="number"
              step="0.01"
              value="${config.strategy.demoKalshiUnderlyingDollars.toFixed(2)}"
            />
          </div>
...
app.get('/api/demo-market-seed', async (_req, res) => {
  ...
  try {
    ...
    const prefix = config.kalshi.marketTicker.split('-')[0].toUpperCase();
    ...
    logger.warn('Dashboard /api/demo-market-seed failed', { status, message });
...
app.post('/api/start', requireDashboardAuth, async (req, res) => {
  ...
  try {
    ...
    logger.warn('Dashboard /api/start failed', { status, message });
    res.status(status).json({ error: message });
  }
});
...
app.post('/api/stop', requireDashboardAuth, async (_req, res) => {
  try {
    ...
    const message = e instanceof Error ? e.message : String(e);
    const status = httpStatusFromKalshiError(e) ?? 500;
    logger.warn('Dashboard /api/stop failed', { status, message });
    res.status(status).json({ error: message });
  }
});
