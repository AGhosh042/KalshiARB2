...
exports.parseFixedPointDollars = parseFixedPointDollars;
exports.contractDollarsToCents = contractDollarsToCents;
exports.isPlausibleBtcUsd = isPlausibleBtcUsd;
exports.parseUnderlyingBtcFromLastPriceField = parseUnderlyingBtcFromLastPriceField;
exports.resolveExpirationValueDollars = resolveExpirationValueDollars;
exports.normalizeTradableStatus = normalizeTradableStatus;
exports.buildKalshiMarketFromRaw = buildKalshiMarketFromRaw;
 ...
 */
function parseUnderlyingBtcFromLastPriceField(raw) {
    const n = parseFixedPointDollars(raw);
    if (n === undefined)
        return undefined;
    if (n <= MAX_CONTRACT_LAST_TRADE_USD)
        return undefined;
    if (!isPlausibleBtcUsd(n))
        return undefined;
    return n;
}
/**
 ...
 */
function resolveExpirationValueDollars(m) {
    ...
    if (floor !== undefined && cap === undefined)
        return floor;
    if (cap !== undefined && floor === undefined)
        return cap;
    const rules = String(m['rules_primary'] ?? '');
    const match = rules.match(/(?:above|below|between)\s+([\d,]+(?:\.\d+)?)(?:\s*-\s*([\d,]+(?:\.\d+)?))?/i);
    if (match) {
 ...
 */
function normalizeTradableStatus(raw) {
    if (raw === 'active' || raw === 'open')
        return 'open';
    if (raw === 'settled' || raw === 'determined')
        return 'settled';
    if (raw === 'finalized')
        return 'finalized';
    return 'closed';
}
function parseExpirationValueField(m) {
...
function buildKalshiMarketFromRaw(m) {
    ...
    const yes_bid = typeof m['yes_bid'] === 'number'
        ? m['yes_bid']
        : yesBidD !== undefined
            ? contractDollarsToCents(yesBidD)
            : 0;
    const yes_ask = typeof m['yes_ask'] === 'number'
        ? m['yes_ask']
        : yesAskD !== undefined
            ? contractDollarsToCents(yesAskD)
            : 0;
    const no_bid = typeof m['no_bid'] === 'number'
        ? m['no_bid']
        : noBidD !== undefined
            ? contractDollarsToCents(noBidD)
            : 0;
    const no_ask = typeof m['no_ask'] === 'number'
        ? m['no_ask']
        : noAskD !== undefined
    ...
    const expiration_value_dollars = parseExpirationValueField(m) ?? resolveExpirationValueDollars(m);
...
//# sourceMappingURL=marketNormalize.js.map